...
async function main() {
  ...
  console.log({ sshDestinationString });

  const sshDestination = await parseSshDestination(sshDestinationString, {
    user: "root",
  });
  console.log({ sshDestination });

  const firstLine = await connectAndGetFirstLine(sshDestination);
  console.log({ firstLine });

  const sshServerType = determineSshServerType(firstLine);
  console.log({ sshServerType });

  const passphrase = await readFirstLine(Deno.stdin.readable);
  console.log(`The passphrase is ${passphrase.length} characters long.`);

  if (sshServerType === "dropbear") {
    dropbearAutoUnlock(sshDestination);
  } else {
    throw new Error(`Unsupported ssh server type: ${sshServerType}`);
  }
}

function dropbearAutoUnlock(_destination: SshDestination) {
